...
describe("registryVocabulary — carry loading modes corrected to match documentation", () => {
  ...
  test("6. no other pilot registry entry's supportedLoadingModes changed", () => {
    const EXPECTED_LOADING_MODES: Record<string, readonly string[]> = {
      ...
      bear_crawl: ["bodyweight"],
      bridging: ["bodyweight"],
      footwork_drills: ["bodyweight"],
      ...
      // Registry Lot 6 — Sprint intervals. First and only entry using
      // "locomotion_only", per 33_EXERCISE_PRESCRIPTION_CAPABILITIES.md's
      // own "Exercise Family 10 — Sprints and Locomotion". Deliberately not
      // "bodyweight": requiring no equipment and being a bodyweight
      // exercise are two different statements.
      sprint_intervals: ["locomotion_only"],
      ...
      // Registry Lot 12 — Battle ropes. The only entry using "rope", a
      // pre-existing LoadingMode value whose meaning is exactly a rope the
      // athlete sets in motion. Deliberately not "bodyweight" (an implement
      ...
      // Registry Lot 14 — Towel pull-up. "# Movement Description" documents
      // the athlete's own bodyweight; the weighted and assisted variations
      // are documented but not represented by this entry.
      towel_pull_up: ["bodyweight"],
      // Registry Lot 15 — the two rope entries. rope_climb moves the
      // athlete's own bodyweight up a rope; rope_pull draws an external
      // resistance along one, which is the `rope` LoadingMode's own meaning
...
const DOSE_NARROWING_EXCEPTIONS = [
  ...
  "hang_power_clean",
  // Registry Lot 3 — Movement immediate: bear_crawl, bridging,
  // technical_stand_up and shrimping all narrow exerciseDoseConstraints
  // (sets pinned/narrowed) against the shared controlled_mobility_sets_v0_1
  // profile. footwork_drills and shadow_boxing are deliberately NOT added
  // here - both declare exerciseDoseConstraints: null (their own documented
  // session-length volume exceeds what the shared profile can represent;
  // see the block comments in exercisePrescriptionRegistry.ts).
  "bear_crawl",
  "bridging",
  "technical_stand_up",
  "shrimping",
  ...
  "rowerg_intervals",
  // Registry Lot 6 — Sprint intervals: sprint_intervals narrows the
  // per-interval work duration only (5-8s, from its own "Typical Work
  // Duration: 5-10 seconds" intersected with
  // repeated_sprint_intervals_v0_1's own [3, 8]).
  "sprint_intervals",
  // Registry Lot 7 — Core repetition work: ab_wheel declares its own
  // documented "2 to 5 sets" and "3 to 12 repetitions" against
  ...
  // Registry Lot 12 — Battle ropes: narrows the interval count (5-12) from
  ...
  // Registry Lot 14 — Towel pull-up: declares the union of its chapter's two
  // repetition prescriptions (sets 3-5, repetitions 2-8). Both match the
  // module envelope exactly and are declared so the entry states its own
  // documented range.
  "towel_pull_up",
  // Registry Lot 15 — each rope entry declares its own documented sets and
...
const INTENSITY_CONSTRAINT_EXCEPTIONS = [
  ...
  "heavy_bag_power_intervals",
  // Registry Lot 12 — the same shared profile, the OTHER rule. battle_ropes
...
const REST_CONSTRAINT_EXCEPTIONS = [
  ...
  "assault_bike_intervals",
  // Registry Lot 14 — declares the union of its two documented rest windows
  // (90-240 s), which IS the module envelope, stated rather than inherited.
  "towel_pull_up",
  // Registry Lot 15 — each declares its own chapter's documented window.
  "rope_climb",
  "rope_pull",
] as const;
 ...
 */
const BETWEEN_SETS_REST_EXCEPTIONS = [
  "dead_bug",
  "hanging_leg_raise",
  "plate_pinch",
  "towel_pull_up",
