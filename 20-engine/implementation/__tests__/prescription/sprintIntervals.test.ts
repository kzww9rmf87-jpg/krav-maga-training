...
describe("sprint_intervals — registry, knowledge base and profile counts", () => {
  ...
  test("5. no other exercise was added: the 60 previous ids plus sprint_intervals account for every key", () => {
    ...
    const ADDED_BY_LATER_LOTS = ["ab_wheel", "dead_bug", "hanging_leg_raise", "plate_pinch", "heavy_bag_power_intervals", "battle_ropes", "assault_bike_intervals", "towel_pull_up"] as const;
...
describe("sprint_intervals — determinism, non-mutation and non-regression", () => {
  ...
  test("30. no regression on the 60 previous entries: each still prescribes with its own declared equipment", () => {
    ...
    const ADDED_BY_THIS_OR_LATER_LOTS: readonly string[] = [EXERCISE_ID, "ab_wheel", "dead_bug", "hanging_leg_raise", "plate_pinch", "heavy_bag_power_intervals", "battle_ropes", "assault_bike_intervals", "towel_pull_up"];
