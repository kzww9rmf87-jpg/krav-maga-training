...
describe("plate_pinch — registry, knowledge base, profile and equipment counts", () => {
  ...
  test("3. the numerical profiles went from 16 to 17 — ISO-GRIP, implemented once, by the preceding commit; later lots added INT-POWER then GRIP-REPETITION-STRENGTH, bringing the total to 21", () => {
  ...
  test("6. no other exercise was added: the 64 previous ids plus plate_pinch account for every key", () => {
    ...
    const ADDED_BY_LATER_LOTS = ["heavy_bag_power_intervals", "battle_ropes", "assault_bike_intervals", "towel_pull_up", "rope_climb", "rope_pull"] as const;
...
describe("plate_pinch — determinism, non-mutation and non-regression", () => {
  ...
  test("33. no regression on the 64 previous entries: each still prescribes with its own declared equipment", () => {
    ...
    const ADDED_BY_THIS_OR_LATER_LOTS: readonly string[] = [EXERCISE_ID, "heavy_bag_power_intervals", "battle_ropes", "assault_bike_intervals", "towel_pull_up", "rope_climb", "rope_pull"];
